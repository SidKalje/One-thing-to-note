...
def process_article(selected_article):
    try:
        ...
        return None
    summary_prompt = (
        "Summarize the text delimited by triple quotes in 350 words at maximum.\n\n'''"
    )
    summary_prompt += (
        "Headline: " + article.title + "\n\nArticle:" + article.text + "'''"
    )
    developer_msg_summary = (
        "You are an expert news summarizer tasked with condensing an article into a concise summary of 350 words or fewer in an easy-to-read format. "
        ...
        "3. Keep the overall summary concise, limiting it to 350 words, but no less than 300 words. "
