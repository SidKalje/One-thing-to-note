...
const mockNews = [
  {
    id: 1,
    headline: "Breaking: Major Tech Breakthrough",
    summary:
  ...
  },
  {
    ...
    summary: "World leaders reach a historic agreement on reducing carbon emissions in a bid to combat climate change.",
    thumbnail: "/placeholder.svg?height=200&width=300",
    category: "Politics",
  },
  {
  ...
  },
  {
    ...
    summary: "In a thrilling upset, the underdog team clinches the championship title, ending a 50-year drought.",
    thumbnail: "/placeholder.svg?height=200&width=300",
    category: "Sports",
  },
  {
...
const NewsContainer = () => {
  ...
  return (
    <div className={styles.container} ref={containerRef}>
      {Array.from({ length: Math.ceil(mockNews.length / 2) }, (_, pageIndex) => (
        <div
          key={pageIndex}
          className={`${styles.newsPage} ${pageIndex === Math.floor(mockNews.length / 2) ? styles.lastPage : ""}`}
        >
          {pageIndex === Math.floor(mockNews.length / 2) ? (
            ...
            <>
              <NewsCard {...mockNews[pageIndex * 2]} />
              {mockNews[pageIndex * 2 + 1] && <NewsCard {...mockNews[pageIndex * 2 + 1]} />}
            </>
          )}
        </div>
      ))}
      <div className={styles.pageIndicator}>
        {Array.from({ length: Math.ceil(mockNews.length / 2) }, (_, index) => (
          <div key={index} className={`${styles.dot} ${index === currentPage ? styles.active : ""}`} />
        ))}
      </div>
    </div>
  )
